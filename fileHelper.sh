...
The FileHelper Bash Script is for renaming a series of files and creating dummy files in a convenient way for the developing process.
  Options:
    ...
    -r,     Rename series of same type files.(You can also just use -r without any argument, then it will go into read user input mode.)
                argument$1, a series of targeted rename files, (required, you must use comma \",\" as delimiter, like: t1.txt,t2.txt ).
...
renameFile(){
    ...
    if [[ ${#_argList[@]} > 0 ]]; then
        ...
        [[ ! -z "${_argList[1]}" ]] &&  _newFileName=${_argList[1]} || $(  _newFileName="File" && printf "\033[34;1mInput Argument Error, Unvalid customized file name. Go with default output file name, \"File\". \033[0m\n" )

        if [ ! ${_argList[2]} > 0 ]; then _ordinalNumber=1 && printf "\033[34;1mInput Argument Error, Unvalid ordinal number. Go with default initial ordinal number, 1. \033[0m\n"; else _ordinalNumber=$(expr ${_argList[2]} + 0); fi
    else
        echo "Please input the target file(s) you want to rename, please use comma \",\" to be delimiter."
        read target
        _targetFile=$target
        while [[ -z "$_targetFile" ]]; do
            echo "You inputed unvalid string value to be the target file name, please try again  and use comma \",\" to be the delimiter."
            read -e target
            _targetFile=$target
        done
...
function createDummyFile () {
    ...
    _content=$( [ ! -z "${_argList[0]}" ] && echo "${_argList[0]}" || echo "This is test content." )
    _linNum=1

    [[ ${_argList[2]} == 'N' || ${_argList[2]} == 'n' || ${_argList[2]} == 0 ]] && _linNum=0 
    # let user input the content as $1, loop times as $2 and output file name as $3.
    printf "Generate a dummy test data files, ${_argList[3]:-dummyTest.txt}\n"
    for (( i=0;i<${_argList[1]:-20};i++ ))
    do 
        echo "$( [[ $_linNum == 1 ]] && echo "$(( $i+1 )): " )$_content" >> ${_argList[3]:-dummyTest.txt}
    done 
}
...
function createNumDummyFiles () {
    ...
    # let user input the content as $1, loop times as $2 and output file name as $3.
    printf "Generate a series of dummy test data files, ${_argList[3]:-dummyTest.txt}\n"
    for (( f=1; f<=${_argList[4]:-5}; f++ ))
    do    
        echo "  Generate No.$f test data file, $( [ -z "${_argList[3]}" ] && echo "dummyTest_$f.txt" || echo "${_argList[3]%.*}_$f.${_argList[3]#*.}" )"
        for (( i=0;i<${_argList[1]:-20};i++ ))
        do 
            echo "$( [[ $_linNum = 1 ]] && echo "$(( $i+1 )): " )$_content" >> $( [ -z "${_argList[3]}" ] && echo "dummyTest_$f.txt" || echo "${_argList[3]%.*}_$f.${_argList[3]#*.}" )
        done 
    done
}
